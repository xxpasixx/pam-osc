...
- **BUG-1 (High, ReDoS) fixed.** `mapperToOnOff` now rejects any `buttonFeedbackMapper` longer than `MAX_MAPPER_LENGTH` (300 chars) *before* running `V1_MAPPER_PATTERN.exec`, falling back to `on-off` 127/0 + warning. The real v1 mapper is ~90 chars, so legitimate mappers (even with generous whitespace) are unaffected; a megabyte-scale adversarial string can no longer feed the quadratic-backtracking regex and freeze the main process. Regression test asserts a 1 MB evil string resolves in <100 ms.
