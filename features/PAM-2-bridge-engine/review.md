...
- [ ] **BUG-1 (High)** and the M-cluster below — untrusted _device definition_ files are the one gap
...
- **L2 (informational):** `command` strings are arbitrary MA3 console commands by design (v1 parity) — the format's "no executable code" means no code _executed by pam-osc_; a doc note that shared mappings should be reviewed is recommended.
