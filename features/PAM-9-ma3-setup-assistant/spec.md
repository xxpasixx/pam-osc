...
- [ ] **Bundled OSC config completeness** — the exported `gma3_library/inout/osc/pam-osc.xml` currently holds only the **Receive** entry (`pam-osc-recive`); the **Send** entry named `pam-osc` is not in the export yet, so importing it alone does not set up feedback. Need the final export with **both** entries (and verified toggles/ports) before the AC-2b one-click OSC install is truly turnkey. Until then the guide's manual step 2 is the source of truth. [NEEDS CLARIFICATION: final OSC-config export from the maintainer]
