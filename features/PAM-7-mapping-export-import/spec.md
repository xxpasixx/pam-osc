...
- [ ] **AC-14** — Given the app, when I open the native OS menu bar (macOS: top bar; Windows/Linux: window menu), then a **File** menu offers Import Mapping…, Import Board…, Export Mapping ▸ (one entry per catalog mapping), Export Board ▸ (one entry per board), and Export Support Package…; the export submenus reflect the current catalog, and menu-driven imports update the open UI (catalog + notices) like their in-app buttons. _(Delta 2026-07-17: maintainer request — sharing actions in the OS menu bar.)_
