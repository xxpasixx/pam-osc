...
describe("validateDraft (AC-6)", () => {
  ...
  it("rejects malformed addresses and empty address", () => {
    for (const address of ["", "  ", "192.168.0.999", "with space", "ip:9000", "-bad-"]) {
      ...
      expect(
        errors.some((error) => error.field === "console.address"),
        `address "${address}"`
      ).toBe(true);
    }
  });
  ...
  it("accepts IPv4 and hostnames", () => {
    for (const address of ["127.0.0.1", "10.0.0.2", "console.local", "ma3-console", "onpc.example.com"]) {
      const errors = validateDraft(draft({ console: { address, sendPort: 9003, receivePort: 9004 } }), IDS);
      expect(
        errors.filter((error) => error.field === "console.address"),
        `address "${address}"`
      ).toEqual([]);
    }
  });
  ...
  it("rejects unknown mapping ids, duplicate ids, and duplicate ports across mappings", () => {
    const errors = validateDraft(
      draft({
        activeMappings: [
          ...
          { id: "map-a", input: "Port D" },
        ],
      }),
      IDS
    );
    expect(
      errors.some((error) => error.field === "mapping:map-b.input" && error.message.includes("already used"))
    ).toBe(true);
  ...
  it("rejects a missing input port", () => {
    const errors = validateDraft(draft({ activeMappings: [{ id: "map-a", input: "" }] }), IDS);
    expect(
      errors.some((error) => error.field === "mapping:map-a.input" && error.message.includes("Pick a MIDI input"))
    ).toBe(true);
  });
});
